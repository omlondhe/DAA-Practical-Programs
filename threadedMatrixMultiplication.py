...
def takeUserInput() -> list[int, list[list[int]], list[list[int]]]:
    ...
    A: list[list[int]] = [
        [
            int(num)
            for num in input(f"Enter the numbers for row {i + 1} of matrix A:\t").split(
                " "
            )
        ]
        for i in range(N)
    ...
    B: list[list[int]] = [
        [
            int(num)
            for num in input(f"Enter the numbers for row {i + 1} of matrix B:\t").split(
                " "
            )
        ]
        for i in range(N)
...
def multiThreadedMultiplication(
    N: int, A: tuple[tuple[int]], B: tuple[tuple[int]], C: list[list[int]]
) -> None:
